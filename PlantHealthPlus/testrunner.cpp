...
{
    // Chart functionality temporarily disabled for initial working version
    // TODO: Re-enable when chart issues are resolved
}
...
void TestRunnerDialog::exportResults()
{
    ...
    QString terminalOutput = QString("\n=== Plant Health Plus Test Results ===\n");
    terminalOutput += QString("Timestamp: %1\n").arg(QDateTime::currentDateTime().toString());
    terminalOutput += QString("Total Tests: %1\n").arg(totalTests);
    terminalOutput += QString("Passed: %1\n").arg(passedCount);
    terminalOutput += QString("Failed: %1\n").arg(failedCount);
    terminalOutput += QString("Success Rate: %1%%\n").arg(QString::number((double)passedCount / totalTests * 100, 'f', 1));
    terminalOutput += QString("Files exported to: %1\n").arg(resultsDir);
    
    // Print to stdout (will appear in terminal when run from command line)
    qDebug().noquote() << terminalOutput;
    
    // Show success message
    QMessageBox::information(this, "Export Complete", 
                           QString("Test results exported to:\n%1\n\nFiles created:\n- HTML Report\n- JSON Report\n- Terminal output displayed").arg(resultsDir));
}
